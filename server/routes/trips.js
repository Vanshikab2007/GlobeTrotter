...
function getTripWithDetail(tripId) {
  ...
  if (!trip) return null;
  const stops = db.prepare('SELECT * FROM stops WHERE trip_id = ? ORDER BY order_index, start_date').all(tripId);
...
function computeBudget(trip) {
  ...
  // Simple heuristic estimates for stay/transport so the breakdown feels complete,
  // clearly labeled as estimates (not real bookings) — a natural next step for a later milestone.
  const totalNights = trip.stops.reduce((sum, s) => {
    if (!s.start_date || !s.end_date) return sum;
    const nights = Math.max(1, Math.round((new Date(s.end_date) - new Date(s.start_date)) / 86400000));
    return sum + nights;
  }, 0);
  const estimatedStay = totalNights * 60; // $60/night placeholder average
  const estimatedTransport = trip.stops.length * 120; // $120 per inter-city hop placeholder
